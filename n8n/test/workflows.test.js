...
const files = ['crypto-longterm.n8n.json', 'swing-setup.n8n.json', 'stocks-commodities.n8n.json'];
...
for (const f of files) {
  test(`${f}: structure, connections and embedded code`, () => {
    ...
    assert.ok(trigger && trigger.parameters.rule.interval[0].expression, 'cron trigger present');
    const codeNodes = wf.nodes.filter(n => n.type === 'n8n-nodes-base.code');
    assert.ok(codeNodes.length >= 3, 'has Split Assets / Compute / Split Alerts code nodes');
